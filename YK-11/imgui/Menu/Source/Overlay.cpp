...
#pragma comment(lib, "d3dcompiler.lib")  // if you later compile shaders
...
void Overlay::CreateOverlay()
{
	...
	overlay = CreateWindowEx(
		...
		GetSystemMetrics(SM_CYSCREEN), // 1440
		NULL,
		NULL,
		wc.hInstance,
		NULL
...
bool Overlay::RenderMenu = false;   // <-- THIS fixes the unresolved symbol // <-- THIS fixes the unresolved symbol
bool esp = false;


void Overlay::Render() {
	ImGui::SetNextWindowSize({ 100, 60 });
	ImGui::Begin("YK-11", &RenderMenu, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize);
